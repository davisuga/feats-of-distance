 ...
 *)
  ...
  let get_an_artist  ~token ~id =
    let open Lwt.Infix in
    let uri = Request.build_uri "/artists/{id}" in
    let headers = Request.default_headers_with_token token in
    let uri = Request.replace_path_param uri "id" (fun x -> x) id in
    Cohttp_lwt_unix.Client.call `GET uri ~headers >>= fun (resp, body) ->
    Request.read_json_body_as
      (JsonSupport.unwrap Artist_object.of_yojson)
      resp body

  let get_an_artists_albums  ~token ~id ?include_groups ?market ?(limit = 20l)
      ?(offset = 0l) () =
    let open Lwt.Infix in
    ...
    let uri = Request.replace_path_param uri "id" (fun x -> x) id in
    let uri =
      Request.maybe_add_query_param uri "include_groups"
        (fun x -> x)
        include_groups
    in
    ...
    Cohttp_lwt_unix.Client.call `GET uri ~headers >>= fun (resp, body) ->
    Request.read_json_body_as
      (JsonSupport.unwrap Albums_paging_object.of_yojson)
      resp body

  let get_an_artists_related_artists  ~token ~id =
    let open Lwt.Infix in
    ...
    Cohttp_lwt_unix.Client.call `GET uri ~headers >>= fun (resp, body) ->
    Request.read_json_body_as
      (JsonSupport.unwrap Get_multiple_artists_200_response.of_yojson)
      resp body

  let get_an_artists_top_tracks  ~token ~id ?market () =
    let open Lwt.Infix in
  ...
  let get_followed  ~token ~_type ?after ?(limit = 20l) () =
    let open Lwt.Infix in
    let uri = Request.build_uri "/me/following" in
    let headers = Request.default_headers_with_token token in
    let uri =
  ...
  let get_multiple_artists  ~token ~ids =
    let open Lwt.Infix in
  ...
  let get_users_top_artists ~token  ?(time_range = "medium_term") ?(limit = 20l)
      ?(offset = 0l) () =
    let open Lwt.Infix in
    let uri = Request.build_uri "/me/top/artists" in
    let headers = Request.default_headers_with_token token in
    let uri =
      Request.add_query_param uri "time_range" (fun x -> x) time_range
    in
    let uri = Request.add_query_param uri "limit" Int32.to_string limit in
    let uri = Request.add_query_param uri "offset" Int32.to_string offset in
    Cohttp_lwt_unix.Client.call `GET uri ~headers >>= fun (resp, body) ->
