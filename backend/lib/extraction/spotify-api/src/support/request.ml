...
let add_token token headers= add_string_header headers "Authorization" ("Bearer " ^ token)
...
val add_string_header :
  Cohttp.Header.t -> string -> string -> Cohttp.Header.t
...
val maybe_add_header_multi :
  Cohttp.Header.t ->
  string -> ('a -> string list) -> 'a option -> Cohttp.Header.t
...
val read_json_body_as_list_of :
  (Yojson.Safe.t -> 'a) ->
  Cohttp.Response.t -> Cohttp_lwt.Body.t -> 'a list Lwt.t
val read_json_body_as_map :
  Cohttp.Response.t ->
  Cohttp_lwt.Body.t -> (string * Yojson.Safe.t) list Lwt.t
val read_json_body_as_map_of :
  (Yojson.Safe.t -> 'a) ->
  Cohttp.Response.t -> Cohttp_lwt.Body.t -> (string * 'a) list Lwt.t
