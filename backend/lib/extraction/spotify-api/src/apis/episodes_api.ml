...
let get_a_shows_episodes ~id ?market ?(limit = 20l) ?(offset = 0l) () =
  ...
  Request.read_json_body_as
    (JsonSupport.unwrap Episodes_paging_object.of_yojson)
    resp body
...
let get_an_episode ~id ?market () =
  ...
  Cohttp_lwt_unix.Client.call `GET uri ~headers >>= fun (resp, body) ->
  Request.read_json_body_as
    (JsonSupport.unwrap Episode_object.of_yojson)
    resp body
...
let get_multiple_episodes ~ids ?market () =
  ...
  Cohttp_lwt_unix.Client.call `GET uri ~headers >>= fun (resp, body) ->
  Request.read_json_body_as
    (JsonSupport.unwrap Get_multiple_episodes_200_response.of_yojson)
    resp body
