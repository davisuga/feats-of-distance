...
let get_an_album ~id ?market () =
  ...
  Request.read_json_body_as
    (JsonSupport.unwrap Album_object.of_yojson)
    resp body
...
let get_an_artists_albums ~id ?include_groups ?market ?(limit = 20l)
    ?(offset = 0l) () =
  let open Lwt.Infix in
...
let get_multiple_albums ~ids ?market () =
  ...
  Cohttp_lwt_unix.Client.call `GET uri ~headers >>= fun (resp, body) ->
  Request.read_json_body_as
    (JsonSupport.unwrap Get_multiple_albums_200_response.of_yojson)
    resp body
...
let get_new_releases ?country ?(limit = 20l) ?(offset = 0l) () =
  ...
  Cohttp_lwt_unix.Client.call `GET uri ~headers >>= fun (resp, body) ->
  Request.read_json_body_as
    (JsonSupport.unwrap Get_new_releases_200_response.of_yojson)
    resp body
